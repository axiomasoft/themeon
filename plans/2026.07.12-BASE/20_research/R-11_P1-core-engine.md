...
> ⚠️ **ОПРОВЕРГНУТО (2026-07-14, P8)**: строковая форма `color` НЕ валидна против спеки 2025.10.
> Прогон стороннего валидатора `@terrazzo/parser@2.4.0` (не мнение) на `color.legacy:
> 'hsl(210 40% 50%)'` даёт `core/valid-color: Migrate to the new object format` — INVALID. Это
> утверждение — один из корней Blocker/Major находок P8 (`toDTCG` эмитил невалидный DTCG).
> Канон — `findings/P8-dtcg-2025-10-canon.md` §S4; решение — **P-D60**.
...
> ⚠️ **ОПРОВЕРГНУТО (2026-07-14, P8)**: `dimension` представим спекой 2025.10 ТОЛЬКО как объект
> `{value, unit}` с `unit ∈ {px, rem}` — легаси-строка и произвольные единицы (`%`/`em`/`vh`/
> `calc()`) не валидны и не эмитятся структурно вообще (пропуск + warning + `$extensions`-мост).
> Канон — `findings/P8-dtcg-2025-10-canon.md` (находка аудита #6); решение — **P-D60**.

...
export async function runExplain(opts: ExplainOptions): Promise<{ exitCode: number; output: string }> {
  ...
  const payload = buildExplainPayload(ctx, opts.token)
...
export const explainCommand = defineCommand({
  meta: {
    name: 'explain',
    description: 'Explain a token path: aliases, CSS variable, and theme overrides',
  },
  args: {
    token: {
      type: 'positional',
      description: 'Dot-separated token path (e.g. color.text)',
      required: true,
    },
    config: {
      type: 'string',
  ...
  async run({ args }) {
    try {
      ...
      const { exitCode, output } = await runExplain({
        ...
        token: args.token,
        format,
        refLayer: args['ref-layer'] as ExplainOptions['refLayer'],
